...
public class CharacterEntity {
	...
	public HashMap<String, BufferedImage[]> animatorMap = new HashMap<String, BufferedImage[]>(); // left, right, up,
																									// down, upLeft,
																									// upRight,
																									// downLeft,
																									// downRight;
	// public String[] directions = {"idle", "walkF", "walkFL", "walkFR", "walkL",
	// "walkR", "walkB", "walkBL", "walkBR"};
		...
		for (int i = 0; i < directions.length; i++) {
			if (directions[i].equals("idle")) {
				if (!containsIdle)
					continue;
				for (int j = 0; j < 3; j++) {
					// String imagePath = "res\\images\\characters\\" + characterType + " " +
					// characterMode + "\\Idle\\" + directions[i] + j + ".png";
					String imagePath = "resources/images/characters/" + characterType + " " + characterMode + "/Idle/"
							+ directions[i] + j + ".png";
				...
				for (int j = 0; j < animatorFrames; j++) {
					String imagePath = "resources/images/characters/" + characterType + " " + characterMode + "/Walk/"
							+ directions[i] + j + ".png";
					if (!animatorMap.containsKey(directions[i])) {
